# https://raw.githubusercontent.com/zorzi-s/PolyWorldPretrainedNetwork/19c847848bb7a4b2221e6ff3cfb8da627849141f/models/matching.py
...
from corner_utils import scores_to_permutations, permutations_to_polygons
...
class AttentionalGNN(nn.Module):
    def __init__(self, feature_dim: int, num_layers: int):
        ...
        self.conv_offset = nn.Sequential(
            ...
            nn.BatchNorm1d(feature_dim),
            nn.ReLU(inplace=True),
            nn.Conv1d(feature_dim, 2, kernel_size=1,
                      stride=1, padding=0, bias=True),
            nn.Hardtanh()
        )
...
class ScoreNet(nn.Module):
    def __init__(self, in_ch):
        ...
        self.conv1 = nn.Conv2d(in_ch, 256, kernel_size=1,
                               stride=1, padding=0, bias=True)
...
class OptimalMatching(nn.Module):
    ...
    def predict(self, image, descriptors, graph):
        ...
        # out: normalized coordinate system [-1, 1]
        norm_graph = self.normalize_coordinates(graph, W, input="global")
        ...
        # out: global coordinate system [0, W]
        graph = self.normalize_coordinates(norm_graph, W, input="normalized")
